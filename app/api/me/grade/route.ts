   ...
   try {
      ...
      if (grade === '별') {
         ...
         if (grade === '로켓') {
            ...
            const targetPoints = topUser[0]?.points ?? currentUser.points;
            nextGradeInfo = {
               next: 'UFO',
            ...
            // 행성 -> 로켓 (상위 60%에 진입)
            const top60Index = Math.floor(totalEligible * 0.6);
            const targetUser = await prisma.user.findMany({
               ...
               orderBy: { points: 'desc' },
               skip: Math.max(0, top60Index - 1),
               take: 1,
               select: { points: true },
            });

            const targetPoints = targetUser[0]?.points ?? currentUser.points;
            nextGradeInfo = {
               next: '로켓',
               need: Math.max(0, targetPoints - currentUser.points + 1),
            };
