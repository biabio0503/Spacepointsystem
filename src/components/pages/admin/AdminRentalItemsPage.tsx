...
import type { RentalItem } from '../../context/AppContext';
import { motion, AnimatePresence } from 'motion/react';
import { Plus, Edit2, Trash2, Package, X, Save } from 'lucide-react';

export default function AdminRentalItemsPage() {
  const { rentalItems, addRentalItem, updateRentalItem, deleteRentalItem } = useApp();
    ...
    <div className="min-h-screen pb-20" style={{ background: '#F0F2F8' }}>
      ...
      <div className="px-5 py-4 grid grid-cols-3 gap-3">
        <div className="bg-white p-4 rounded-xl text-center">
          <p className="text-2xl font-bold text-blue-900">{rentalItems.length}</p>
          <p className="text-xs text-gray-600 mt-1">전체 품목</p>
        </div>
        <div className="bg-white p-4 rounded-xl text-center">
          <p className="text-2xl font-bold text-green-600">
            {rentalItems.filter(i => i.isActive).length}
          </p>
          <p className="text-xs text-gray-600 mt-1">활성 품목</p>
        </div>
        <div className="bg-white p-4 rounded-xl text-center">
          <p className="text-2xl font-bold text-orange-600">
            {rentalItems.reduce((sum, i) => sum + i.totalStock, 0)}
          </p>
